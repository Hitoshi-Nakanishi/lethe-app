...
def describe_error(exc: Exception, *, ollama: bool = False) -> str:
...
class App:
    ...
    def refine_transcript(self) -> None:
        ...
        self._set_status(f"Ollama で校正中（{OLLAMA_MODEL}）...", "busy")

        def worker() -> None:
            try:
                from llm.refine import refine_transcript as do_refine

                refined = do_refine(transcript, notes, model=OLLAMA_MODEL, ollama_url=OLLAMA_URL)
                self._refine_queue.put(("ok", refined))
            except Exception as exc:
                self._refine_queue.put(("error", describe_error(exc, ollama=True)))
    ...
    def generate_minutes(self) -> None:
        ...
        self.hq_button.config(state="disabled")
        self._set_status(f"Ollama で議事録を生成中（{OLLAMA_MODEL}）...", "busy")

        def worker() -> None:
            try:
                from llm.summarize import summarize

                md = summarize(transcript, model=OLLAMA_MODEL, ollama_url=OLLAMA_URL)
                self._minutes_queue.put(("ok", md))
            except Exception as exc:
                self._minutes_queue.put(("error", describe_error(exc, ollama=True)))
